...
try:
    import sys
    sys.path.append('\media\akshata\DATA\Python\script\Lib\site-packages')
    exist = False
...
class CarlaGame(object):
    ...
    def _on_new_episode(self):
        ...
        #         BufferedImageSaver('{}/ep_{}/'.format(self.out_dir,self.episode_count),
        #             1000, WINDOW_HEIGHT, WINDOW_WIDTH, 3, 'CameraSemSeg')
        #         ]
    ...
    def _on_loop(self):
        ...
        # save images captured by the cameras to disk
        directory = "CameraRGB"
        path = os.path.join(self.out_dir, directory)

        folder = os.path.dirname(path)
        if not os.path.isdir(folder):
            os.makedirs(folder)

        directory = "CameraSemSeg"
        path = os.path.join(self.out_dir, directory)
        ...
        csv_filename = self.out_dir + "/data.csv"
        ...
        # print("type of map position : ",type(map_position))
        # print("type of lane orientation : ",type(lane_orientation))
        # print("type of car orientation : ",type(rotation_car))
        lane_ori = list(lane_orientation)
        
        data_position = map_position + lane_ori + rotation_car
        ...
        data_position.append(control.steer)
        data_position.append(control.throttle)

        #print("frame count : ", self.frame_count)
        # print("episode count : ",self.episode_count)
        # data_position.append(self.frame_count)
        # data_position.append(self.episode_count)

        if self.save_images_to_disk:
            for name, measurement in sensor_data.items():
                if name == 'CameraRGB':
                    print("frame : ",self.frame_count)
                    filename = self.out_filename_format.format(
                        self.episode_count, name, self.frame_count)
                    data_position.append(control.steer)
                    data_position.append(control.throttle)
                    data_position.append(self.frame_count)
                    data_position.append(self.episode_count)
                    ...
                    with open(csv_filename, mode='a') as csv_file:
                        ...
                        csv_writer.writerow(data_position)
                else:
                    filename = self.out_filename_format.format(
                        self.episode_count, name, self.frame_count)
                    # image_sem_seg = image_converter.labels_to_cityscapes_palette(measurement)
                    # measurement.save_to_disk(filename,carla.ColorConverter.CityScapesPalette)
                    # image_sem_seg.save_to_disk(filename)
                    measurement.save_to_disk(filename)
            

        self.frame_count += 1

        # save images using buffered saver
        # if self.save_images_to_disk:
        #     for name, measurement in sensor_data.items():
        #         for image_saver in self.saver:
    ...
    def _print_player_measurements_map(
        ...
        print("in _print_player_measurements_map")
...
def main():
    ...
    log_level = log_levels[args.debug]
